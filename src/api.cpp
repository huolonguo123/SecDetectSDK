...
bool repack_input_ok(const char* input) {
    ...
    std::string apk(input);
    size_t comma = apk.find(',');
    if (comma != std::string::npos) apk.erase(comma);
    /* 去掉首尾空白再判 */
    size_t b = apk.find_first_not_of(" \t");
    size_t e = apk.find_last_not_of(" \t");
    if (b == std::string::npos) return false;
    apk = apk.substr(b, e - b + 1);
    return sec::util::file_exists(apk.c_str());
}

/* ---- DETECT_ALL:逐项跑,命中/有信息才写一行;needs_input 的项
 *      只有调用方给了合法 input 才参与,否则明示 skipped ---- */
int run_all(const char* input, char* out, size_t outlen) {
    ...
    for (BaseDetector* d : registry().items) {
        ...
        bool risk = false;
        if (d->needs_input()) {
            if (repack_input_ok(input)) {
                risk = d->run(input, f);
            } else {
                f.add("repack: skipped (pass apk path as input for this item)");
            }
        } else {
            risk = d->run(nullptr, f);
        }
        if (!all.empty()) all += '\n';
        char hdr[40];
...
SEC_API int secdetect(int detect_type, const char* input, char* output, size_t output_len) {
    ...
    /* repack 依赖 input:缺失或文件打不开都算参数错误 */
    if (d->needs_input() && !repack_input_ok(input)) {
        if (output && output_len) {
            std::string msg = "repack: input required, usage: <apk_path>[,<baseline_sha256>]";
